...
select count(t2.cus_gender) as NoofCustomer, t2.cus_gender as gender from
...
*/
select `order`.*,p.pro_name from `order`
inner join
(select product.pro_name,supplier_pricing.pricing_id from product
inner join
supplier_pricing on
supplier_pricing.pro_id=product.pro_id) as p
on `order`.pricing_id=p.pricing_id where `order`.cus_id=2;


/* practice
select `order`.*,p.pro_name from `order`
inner join
(select product.pro_id,product.pro_name,supplier_pricing.pricing_id from
product
inner join
supplier_pricing 
on supplier_pricing.pro_id=product.pro_id) as p
on `order`.pricing_id=p.pricing_id where `order`.cus_id=2;*/

/*practice
select supplier.*  from supplier
inner join
(select supplier_pricing.supp_id,supplier_pricing.pro_id from 
...
*/
select category.cat_id,category.cat_name,a.pro_name,min(Min_price) as Min_price from category
inner join
(select product.cat_id,product.pro_name,p.supp_price,Min_price from
product
inner join
(select supplier_pricing.pro_id,min(supplier_pricing.supp_price) as Min_price,supplier_pricing.supp_price from supplier_pricing group by supplier_pricing.pro_id)
as p
on product.pro_id=p.pro_id) 
as a on category.cat_id=a.cat_id group by a.cat_id;
...
select product.pro_id as Id,product.pro_name as Name from product
inner join 
(select supplier_pricing.pro_id from supplier_pricing
inner join
(select `order`.pricing_id  from `order` where ord_date>'2021-10-05')
as p on supplier_pricing.pricing_id=p.pricing_id) 
as a on product.pro_id=a.pro_id;
